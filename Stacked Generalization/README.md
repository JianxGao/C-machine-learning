...
将每一个样本的 $n$ 个预测值组成新的 $stack\_row$，从而生成一个新的数据集，再使用一个新的集成模型 $(Aggregator \;Model)$ 对新的数据集进行分类/回归预测，完成算法。
...
下将通过通过多C文件，本节以[声纳数据集](https://goo.gl/FXFWwF)为例，使用SG算法，预测声纳的类别。
